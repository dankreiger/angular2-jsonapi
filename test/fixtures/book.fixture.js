"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var author_fixture_1 = require("./author.fixture");
function getSampleBook(i, authorId) {
    return {
        id: '' + i,
        type: 'books',
        attributes: {
            date_published: author_fixture_1.BOOK_PUBLISHED,
            title: author_fixture_1.BOOK_TITLE,
            created_at: '2016-09-26T21:12:41Z',
            updated_at: '2016-09-26T21:12:41Z'
        },
        relationships: {
            chapters: {
                links: {
                    self: '/v1/books/1/relationships/chapters',
                    related: '/v1/books/1/chapters'
                }
            },
            firstChapter: {
                links: {
                    self: '/v1/books/1/relationships/firstChapter',
                    related: '/v1/books/1/firstChapter'
                }
            },
            author: {
                links: {
                    self: '/v1/books/1/relationships/author',
                    related: '/v1/books/1/author'
                },
                data: {
                    id: authorId,
                    type: 'authors'
                }
            }
        },
        links: {
            self: '/v1/books/1'
        }
    };
}
exports.getSampleBook = getSampleBook;
