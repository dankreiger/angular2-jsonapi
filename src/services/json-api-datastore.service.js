...
var JsonApiDatastore = /** @class */ (function () {
    ...
    JsonApiDatastore.prototype.getRelationships = function (model) {
        var _this = this;
        var relationships;
        var data = model;
        for (var key in data) {
        ...
        this.relationsToBeDeleted(model).forEach(function (toDelete) {
            relationships = relationships || {};
            relationships[toDelete] = { data: null };
        });
        return relationships;
    };
    JsonApiDatastore.prototype.relationsToBeDeleted = function (model) {
        var belongsToMetadata = Reflect.getMetadata('BelongsTo', model);
        if (belongsToMetadata == null) {
            return [];
        }
        return belongsToMetadata.filter(function (entity) { return model.hasOwnProperty(entity.propertyName); })
            .filter(function (entity) { return model[entity.propertyName] === null; })
            .map(function (entity) { return entity.propertyName; });
    };
    ...
    JsonApiDatastore.prototype.updateRelationships = function (model, relationships) {
        ...
        for (var relationship in relationships) {
            if (relationships.hasOwnProperty(relationship) && model.hasOwnProperty(relationship)) {
                ...
                var hasMany = relationshipModel ? Reflect.getMetadata('HasMany', relationshipModel) : false;
